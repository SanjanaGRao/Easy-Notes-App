...
class UserModels {
  /**
   * @description Query to login for a User
   * @param {Object} object
  ...
  loginUser = (object, callback) => {
    return User.findOne({ email: object.email }, (err, data) => {
      return err
        ? callback(err, null)
        : data == null
        ? callback("Email ID isn't present", null)
        : callback(null, data);
    });
  };

  /**
   * @description Query to create a User and save user in the database
   * @param {Object} object
   * @param {callback} callback
   * @returns error or callback
   */
  ...
  /**
   * @description Query to find and update the user details
   * @param {String} findId
   * @param {Object} object
   * @param {callback} callback
  ...
  /**
   * @description Query to find and remove an user id as a parameter
   * @param {String} findId
   * @param {callback} callback
   * @returns
   */
